  ...
  return function(pkg, { incrementAndGetCompletedPackageCount, getTotalPackageCount }) {
    return new Promise((resolve, reject) => {
      ...
      child.on('exit', (code, signal) => {
        logger.debug(`yerna: package '${pkg.name}' task with child pid ${child.pid} exited with code ${code} and signal ${signal} after ${((Date.now() - spawnTime) / 1000).toFixed(2)}s`);
        ...
        if (!isAborting) {
          if (mergedOutput.length) {
            ...
            logger.info(`yerna: completed ${chalk.cyan(pkg.name)} with no output (${incrementAndGetCompletedPackageCount()}/${getTotalPackageCount()})`);
          }
        }
