...
function runPackagesToposorted(logger, commander, packagesPath, packages, runTask) {
  ...
  const pendingDependencies = _.mapValues(packageByName, _.constant(0));

  packages.forEach(pkg => {
    pendingDependencies[pkg.name] = pkg.localDependencies.filter(pkgName => packageByName[pkgName]).length;
  });

  function handleCompletedPackage(pkg) {
    pkg.localDependents
      .filter(pkgName => packageByName[pkgName])
      .forEach(pkgName => {
        pendingDependencies[pkgName]--;
      });

    enqueueAvailablePackages();
  }

  function enqueueAvailablePackages() {
    if (_.size(pendingDependencies) > 0) {
      let freePackages =_.keys(_.pickBy(pendingDependencies, value => value === 0));

      if (freePackages.length === 0 && q.idle()) {
  ...
  const q = async.queue((pkg, callback) => {
    return runTask(pkg, commander.quiet)
      .asCallback(callback)
      .then(() => handleCompletedPackage(pkg))
      .catch(logAndAbort);
  }, commander.parallel);
